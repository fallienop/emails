...
"winnifred56@yahoo.com" };
        ...
        {
            mails[i] = mails[i].Replace('_', '.');


            for (int j = 0; j < mails[i].Length; j++)
            {
                if (char.IsDigit(mails[i][j]))
                {
                    mails[i] = mails[i].Remove(j, 1);
                }
            }
        ...
        foreach (var s in mails)
        {
            ...
            foreach (var c in s.Select((a, b) => (a, b)))
            {
                if (c.a == '.')
                {
                    count++;
            ...
            else
            {
                ...
                int e = s.IndexOf('@');
                int n = s.IndexOf('.');
                int ns = s.LastIndexOf('.');
                Console.ForegroundColor = getRandomColor();
                while (Console.ForegroundColor == ConsoleColor.Black || Console.ForegroundColor == ConsoleColor.Red) Console.ForegroundColor = getRandomColor();



                Console.WriteLine(" Firstname : {0}\n LastName: {1}\n Mail: {2}\n Domain: {3} \n Extention: {4}", s.Substring(0, n), s.Substring(n + 1, e - n - 1), s, s.Substring(e + 1, ns - e - 1), s.Substring(ns));
